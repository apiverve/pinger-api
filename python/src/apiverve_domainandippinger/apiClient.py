...
class PingerAPIClientError(Exception):
...
class ValidationError(PingerAPIClientError):
...
class PingerAPIClient:
    # Validation rules for parameters (generated from schema)
    VALIDATION_RULES = {"host": {"type": "string", "required": True}, "timeout": {"type": "integer", "required": False, "min": 100, "max": 3000, "default": 1500}, "retries": {"type": "integer", "required": False, "min": 1, "max": 5, "default": 1}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the PingerAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises PingerAPIClientError: If API key is invalid
        """
        ...
        self.secure = secure
        self.debug = debug
        self.base_url = 'https://api.apiverve.com/v1/pinger'
        self.headers = {
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[PingerAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns DomainandIPPingerData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises PingerAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise PingerAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise PingerAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise PingerAPIClientError(f"Request failed: {str(e)}")
